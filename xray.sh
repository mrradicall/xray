...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        echo -e "Got x-ui latest version: v1.7.9, beginning the installation..."
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch3xui).tar.gz https://github.com/MHSanaei/3x-ui/releases/download/v1.7.9/x-ui-linux-$(arch3xui).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version="v1.7.9"
        url="https://github.com/MHSanaei/3x-ui/releases/download/v2.0.2/x-ui-linux-$(arch3xui).tar.gz"
